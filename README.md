...
- トラックごとの音源（GM 128音色）・表示・色・音量の設定
- 譜面トラックと表示形式（五線譜・ギターTAB・ドラム譜）の切り替え。ギターTABは標準チューニングで推定します
- 表示鍵域、落下時間、背景色、速度、音量、ミュート
- GM SoundFontを使ったサンプル音源再生。音源サンプルは選択時に外部から読み込み、MIDIファイル自体はブラウザ外へ送信しません
